...
try {
    ...
    // 예외 처리
    if($result_cnt !==1) { // 영향받은 레코드가 1이 아닐때
        // 개발자의 필요에 따라 강제로 예외 발생 시키는 방법
        throw new Exception("영향 받은 레코드 수 이상"); // catch "getMessage"에 들어갈 문구(에러 메세지)
    }
    
    // 정상 완료 
    $conn->commit(); // transantion 후에 반드시!!
    echo $result_cnt."커밋 완료\n";
} catch (\Throwable $e) {
    if(!empty($conn)) {
        $conn->rollBack(); // transantion 후에 반드시!!
    }
